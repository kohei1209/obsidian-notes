...
> **⚠️ 取得する列に `Status` を必ず含めること。**
> Status が JSON に含まれないと、HTML 側のフィルタリング
> `(l.Status || 'Active') !== 'Cancelled'` が常に Active と判定し、
...
### Step 3: JSON 文字列化（フィールドマッピング付き）

SharePoint の REST API はカラム名に `OData__x0000_` のような内部名を返す場合がある。
HTML テンプレートが期待するキー名と一致させるため、**Select アクションで明示的にマッピング**する。

**Select** アクション（名前: `MappedLogs`）
- From: `body('WorkHoursLog取得')?['value']`
- Map:

| キー（HTML が期待する名前） | 値（SharePoint のフィールド参照） |
|---------------------------|--------------------------------|
| `Title` | `item()?['Title']` |
...
> **⚠️ Status フィールドの参照方法に注意:**
> SharePoint の選択肢列（Choice）は REST API で `{"Value": "Active"}` のような
> オブジェクトとして返される場合がある。
> - **選択肢列の場合**: `item()?['Status']?['Value']` を使用
> - **テキスト列として作成した場合**: `item()?['Status']` を使用
> - 不明な場合は、Flow を一度実行してデータ出力を確認し、正しい参照式を特定すること。
