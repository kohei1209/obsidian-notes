...
> **⚠️ Power Automate の制約**: 「変数の初期化」は条件分岐やループの中に配置できない。
> 先に空文字で初期化してから、条件分岐内で「変数の設定」で上書きする。
...
> **フロー上の見た目（上から順に）**:
> ```
> [変数を初期化する] NewProjectCode = ''
> [変数を初期化する] NewProjectName = ''
> [条件]  length(...) > 0 ?
>   ├─ Yes → [変数の設定] NewProjectCode = ...ProjectCode
>   │        [変数の設定] NewProjectName = ...Title
>   └─ No  → （空）
> ```
...
📅 @{triggerBody()?['subject']}
⏰ @{formatDateTime(triggerBody()?['start'], 'yyyy/MM/dd HH:mm')} 〜 @{formatDateTime(triggerBody()?['end'], 'HH:mm')}（@{variables('NewDurationHours')}h）
