...
登録済みの工数レコードを自動的に同期する。

- **更新**: 時刻・工数を最新値に上書き。プロジェクトを変更したい場合は Teams に再選択カードを送信。
...
- フィルター クエリ:
  ```
  EventId eq '@{triggerBody()?['id']}' and Status eq 'Active'
  ```
- 取得する列: `Id, EventId, ProjectCode, ProjectName, DurationHours`
