...
> `round(..., 2)` で小数点以下2桁に丸める（例: 1.3333... → 1.33）。
...
| DurationHours | `variables('DurationHours')` | round 済み |
...
📅 @{triggerBody()?['subject']}
⏰ @{triggerBody()?['start']} 〜 @{triggerBody()?['end']}（@{variables('DurationHours')}h）
...
6. WorkHoursDashboard/index.html が更新されることを確認
